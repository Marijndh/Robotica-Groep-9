...
def color_bulls_eye(center, ellipse, mean, result):
    bulls_eye = Color('bulls_eye', np.array([20, 0, 0]),
                      np.array([25, 255, 255]))
...
def check_bulls_eye_in_grid(bulls_eyes):
    bulls_eye_cells = []
...
def create_mask(center):
    ...
    # Aangenomen straal van 10 pixels
...
cv.imshow("grid", create_grid_on_image(img, round(width_img / 2),
                                       round(height_img / 2)))
cv.waitKey(0)
